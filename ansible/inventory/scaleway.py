        ...
        for region in regions:
            ...
            for server in api.servers():
                ...
                for server_tag in server['tags']:
                    if 'tag_' + server_tag not in inventory:
                        inventory['tag_' + server_tag] = {'hosts': []}
                    inventory['tag_' + server_tag]['hosts'].append(hostname)
                    grouped = True
                ...
                # private IP
                if server['private_ip'] not in inventory:
                    inventory[server['private_ip']] = {'hosts': []}
                inventory[server['private_ip']]['hosts'].append(hostname)
                ...
                if server['public_ip'] is not None:
                    ...
                    inventory['_meta']['hostvars'][hostname]['ansible_python_interpreter'] = '/usr/bin/python3'

        return inventory
    except Exception:
        # Return empty hosts output
        traceback.print_exc()
        return {'_meta': {'hostvars': {}}, 'all': { 'children': ['ungrouped']}, 'ungrouped': {}}
